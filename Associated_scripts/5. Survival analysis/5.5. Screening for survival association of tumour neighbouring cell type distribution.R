...
hr_function_radii <- function(rad){
  ...
  tma5 <- read_csv("tma5_sqpy_coarse_tumour_touch_count_frac_diff_radii_fine_labels.csv")  %>%
  ...
  write_csv(hr_tma5_6, paste0("HR_top25_v_bot25_tma5+6_sqpy_coarse_tumour_neigh_frac_", rad,"um.csv"))
...
folder_path <- ""
base_pattern <- "HR_top25_v_bot25_tma5\\+6_sqpy_coarse_tumour_neigh_frac_"
...
x1 <- ova_tma_clin %>%
  ...
  left_join(
    frac_ct_rads %>%
      ...
      dplyr::filter(ct %in% c("T cells IL32+", "Monocyte_Macrophages CXCL9+")) %>%
      ...
      mutate(fraction = interaction(.[["T cells IL32+"]], .[["Monocyte_Macrophages CXCL9+"]], sep = "/")),
    by = "sample"
  )
...
surv1 <- ggsurvplot(
  ...
  legend.title = "IL32 / CXCL9", xlab = "Days since diagnosis",
...
# ==============================================================================
# Panel 3F: CXCL13+ T cells & IGFBP5+ Fibroblasts
...
x2 <- ova_tma_clin %>%
  ...
  left_join(
    frac_ct_rads %>%
      dplyr::filter(radius_um == "10") %>%
      dplyr::filter(ct %in% c("T cells CXCL13+", "Fibroblast - IGFBP5+")) %>%
      dplyr::select(sample, ct, count) %>%
      group_by(ct) %>%
      mutate(fraction = ifelse(count > median(count), "High", "Low")) %>%
      ungroup() %>%
      select(-count) %>%
      tidyr::pivot_wider(names_from = ct, values_from = fraction) %>%
      mutate(fraction = interaction(.[["T cells CXCL13+"]], .[["Fibroblast - IGFBP5+"]], sep = "/")),
    by = "sample"
  )
...
surv2 <- ggsurvplot(
  ...
  legend.title = "CXCL13 / IGFBP5", xlab = "Days since diagnosis",
