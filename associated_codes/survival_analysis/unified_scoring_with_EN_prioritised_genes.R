...
## load the coefficients after the elastic-net prioritisation from the 61 survival-associated genes
en_coefs 

coef_df <- data.frame(
  symbol = rownames(en_coefs),
  beta = en_coefs[, 1],
  stringsAsFactors = FALSE
) %>%
  filter(symbol != "(Intercept)", symbol %in% final_coefs)

# -------------------------------------------------------------------------
# calculation of the unified scoring


adj_tum_bulk <- read_csv("all_tumour_merged_tma5_6_combat_adjusted_log2_counts.csv") %>%
...
patient_unified_score_df <- adj_tum_bulk %>%
  ...
  group_by(symbol) %>%
  mutate(z_score = as.numeric(scale(adjusted_log_count))) %>%
  ungroup() %>%
  
  left_join(coef_df, by = "symbol") %>%
  
  mutate(weighted_score = z_score * beta) %>%
